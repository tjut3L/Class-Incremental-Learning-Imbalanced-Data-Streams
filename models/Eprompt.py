...
class EPrompt(nn.Module):
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if self.Q is not None:
                ...
                for i in range(self.pool_size):
                    if i not in self.Q:
                        ...
                        w.append(self.Q[i])
                w = torch.tensor(w)
                w = w.to(similarity.device)
                wight = w.unsqueeze(0).repeat(similarity.shape[0], 1)
                similarity *= wight
            ...
            # self.prompt   3 * 2 * 100 * 5 * 12 * 64  l * n * s * m * h * e
            # sim 24 * 100    b * s
            # prompt l * b * n * s * h * e
            # prompt = torch.einsum('lnshe,bs->lbnshe', self.prompt, similarity)
            out['prompt_idx'] = idx
            if self.use_prefix_tune_for_e_prompt:
            ...
            # 加权求prompt
            batched_prompt = torch.einsum('lnsmhe,bs->lbnmhe', self.prompt, similarity)
            ...
            # Put pull_constraint loss calculation inside 求最大的sim的key和x_embed的相似度 加入到loss中
