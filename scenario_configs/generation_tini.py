...
image_labels = {}
data_split = {}
with open("/home/tjut_zhaoyishuo/clvision-challenge-23/scenario_configs/image_class_labels.txt", 'r') as f:
    q = f.readlines()
    for k in q:
        p = k.split(' ')
        image_labels[int(p[0])] = int(p[1])

with open("/home/tjut_zhaoyishuo/clvision-challenge-23/scenario_configs/train_test_split.txt", 'r') as f:
    q = f.readlines()
    for k in q:
        p = k.split(' ')
        data_split[int(p[0])] = int(p[1])

p_fir = 0.02  # 0.5 0.2 0.02
