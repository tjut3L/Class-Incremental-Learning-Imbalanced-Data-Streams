...
class MyPlugin6(SupervisedPlugin):
    """
    ...
    def before_training_exp(
        ...
        """动态改变LwF的α"""
        self.total = len(strategy.benchmark.seen_classes[strategy.experience.current_experience])
        """ strategy.plugins[0].lwf.alpha = total/new """
        # if self.dic[strategy.experience.current_experience] != 0:
        #     strategy.plugins[0].lwf.alpha = min(40, self.total / self.dic[strategy.experience.current_experience] * 2)
        """ strategy.plugins[0].lwf.alpha = 30/15 """
        # if self.dic[strategy.experience.current_experience] == 0:
        #     strategy.plugins[0].lwf.alpha = 15
        # else:
        #     strategy.plugins[0].lwf.alpha = 30
        """ strategy.plugins[0].alpha = total/old """
        if self.total - self.dic[strategy.experience.current_experience] != 0:
            strategy.plugins[0].lwf.alpha = min(40, self.total / (self.total - self.dic[strategy.experience.current_experience]) * 20)
    ...
    def compute_loss(self, strategy):
        ...
        for i in range(len(self.protos)):
            ...
            alpha = 50 * torch.tensor(self.new_S[self.proto_labels[i]]).to(strategy.device)

            # alpha = 1
            soft_feat = strategy.model.linear(proto.reshape(1,-1)).to(strategy.device).reshape(-1)
            loss_protoaug += alpha * nn.CrossEntropyLoss()(soft_feat, torch.tensor(self.proto_labels[i]).to(strategy.device))
    ...
    def after_eval_exp(
        ...
        """将loss和acc列表保存至文件"""
        if self.count == 49:
            text_txt = 'acc_lwf_proto_net_DA_s3.txt'
            with open(text_txt, 'a') as text:
                ...
                text.write(s)
            text_txt = 'loss_lwf_proto_net_DA_s3.txt'
            with open(text_txt, 'a') as text:
                s = ''
                ...
                text.write(s)
            """将训练好的model保存至文件"""
            with open('Our_lwf_proto_net_DA_s3.pkl', 'wb') as f:
