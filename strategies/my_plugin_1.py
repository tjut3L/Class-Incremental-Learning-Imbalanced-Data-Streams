...
class MyPlugin1(SupervisedPlugin):
    """
    ...
    def compute_loss(self, strategy):
        ...
        for i in range(len(self.protos)):
            ...
            alpha = 30 * torch.tensor(self.new_S[self.proto_labels[i]]).to(strategy.device) # 10
    ...
    def after_eval_exp(
        ...
        """将loss和acc列表保存至文件"""
        if self.count == 49:
            text_txt = 'acc_sub_3_lwf_proto.txt'
            with open(text_txt, 'a') as text:
                ...
                text.write(s)
            text_txt = 'loss_sub_3_lwf_proto.txt'
            with open(text_txt, 'a') as text:
                s = ''
                ...
                text.write(s)
            # """将训练好的model保存至文件"""
            # with open('Our_tiny_lwf_proto_s2.pkl', 'wb') as f:
