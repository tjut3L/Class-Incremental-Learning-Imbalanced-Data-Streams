...
def single_gpu_cotta(model,
    ...
    optimizer = torch.optim.Adam(param_list, lr=0.00006 / 8,
                                 betas=(0.9, 0.999))  # Batchsize=1 now, was 8 during cityscapes training
...
def single_gpu_tent(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            acc_origin = origin_model.forward(return_loss=True, img=data['img'][0],
                                              img_metas=data['img_metas'][0].data[0], gt_semantic_seg=gt)
            acc_adaption = model.forward(return_loss=True, img=data['img'][0], img_metas=data['img_metas'][0].data[0],
                                         gt_semantic_seg=gt)
            ...
            print(seq_name, 'delta_sum', delta_sum, 'acc_delta:', acc_delta, 'acc_origin:', acc_pre, 'loss_adaption:',
                  acc)
        ...
        if isinstance(result, list):
            loss = model.forward(return_loss=True, img=data['img'][0], img_metas=data['img_metas'][0].data[0],
                                 gt_semantic_seg=torch.from_numpy(np.stack(result)).cuda().unsqueeze(1))
            if efficient_test:
                result = [np2tmp(_) for _ in result]
            results.extend(result)
        else:
            loss = model(return_loss=True, img=data['img'][0], img_metas=data['img_metas'][0].data[0],
