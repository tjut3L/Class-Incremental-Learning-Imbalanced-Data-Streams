...
class MyPlugin3(SupervisedPlugin):
    """
    ...
    def compute_loss(self, strategy):
        ...
        for i in range(len(self.protos)):
            ...
            alpha = 50 * torch.tensor(self.new_S[self.proto_labels[i]]).to(strategy.device)# 10
    ...
    def after_eval_exp(
        ...
        """将loss和acc列表保存至文件"""
        if self.count == 49:
            text_txt = 'acc_experience_list_Our_s3_3.txt'
            with open(text_txt, 'a') as text:
                ...
                text.write(s)
            text_txt = 'loss_experience_list_Our_s3_3.txt'
            with open(text_txt, 'a') as text:
                s = ''
                ...
                text.write(s)
            """将训练好的model保存至文件"""
            with open('Our_s3_3.pkl', 'wb') as f:
