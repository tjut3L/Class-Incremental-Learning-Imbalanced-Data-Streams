    ...
    *Notes for subclassing*
    ...
    def _get_indices(self):
        ...
        if self._indices is not None:
            return self._indices.tolist() if type(self._indices) != list else self._indices
        else:
    ...
    def concat(self, other: "FlatData") -> "FlatData":
        ...
        if len(self._datasets) == 1 and len(other._datasets) == 1:
            if self._can_flatten and self._datasets[0] is other:
                ...
                if type(self._get_indices()) != list:
                    print(self._get_indices())
                    m = self._get_indices().tolist()
                    n = other._get_indices().tolist()
                else:
                    m = self._get_indices()
                    n = other._get_indices()
                idxs = m+n
