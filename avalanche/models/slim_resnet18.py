...
from torch.nn.functional import relu, avg_pool2d, sigmoid
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf):
        ...
        self.fc = nn.Linear(512, num_classes * 4, bias=True)
    ...
    def forward(self, x):
        ...
        out = self.linear(out)  # 16*2048  32*32 16*512
...
class MTSlimResNet18(MultiTaskModule, DynamicModule):
    ...
    def __init__(self, nclasses, nf=20):
        ...
        self.linear = MultiHeadClassifier(
            512 * BasicBlock.expansion, nclasses
        )
    ...
    def forward(self, x, task_labels):
        bsz = x.size(0)
        out = relu(self.bn1(self.conv1(x.view(bsz, 3, 256, 256))))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
