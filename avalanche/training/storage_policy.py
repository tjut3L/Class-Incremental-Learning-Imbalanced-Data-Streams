...
class ExperienceBalancedBuffer(BalancedExemplarsBuffer):
    ...
    def update(self, strategy: "SupervisedTemplate", **kwargs):
        ...
        print('----------------------------')
        new_buffer = ReservoirSamplingBuffer(lens[-1])  # 更新Buffer
...
class ERBuffer(BalancedExemplarsBuffer):
    def __init__(
            self,
            max_size: int,
            selection_strategy: Optional["HerdingSelectionStrategy"] = None,
    ):
        """
        :param max_size: The max capacity of the replay memory.
        :param selection_strategy: The strategy used to select exemplars to
    ...
    def getbuffer(self):  # 不返回dataset 返回representation
        return self._buffer

    def update(self, strategy: "SupervisedTemplate", re, y, **kwargs):
        benchmark = strategy.benchmark
        # num_seen_class 要更新
        new_S = self.compute_buffer(benchmark.seen_classes, benchmark.present_classes_in_each_exp, self.buffer_size,
                                    benchmark.num_seen_class, strategy.pre_exp_idx, benchmark.n_samples_table)
        self.updatebuffer(re, new_S, y)
        pass

    def compute_buffer(self, per_seen1, present_class, buffer_size, num_seen_class, exp_idx, n_samples_table):
        '''
        per_seen : P 当前已知的类
        present_class : 当前exprence 的类
        buffer_size : buffer大小
        num_seen_class : 当前已知类的每类的样本数量
        '''
        per_seen = per_seen1[exp_idx]
        for c in present_class[exp_idx]:
            if c.item() in num_seen_class:
                num_seen_class[c.item()] += n_samples_table[c.item()][exp_idx].item()
            else:
                num_seen_class[c.item()] = n_samples_table[c.item()][exp_idx].item()
        ...
        # print(q_)
        for c in per_seen:
            S[c.item()] = math.ceil(q_[c.item()] * buffer_size)
        return S

    def sort_buffer(self):
        for i in self._buffer:
            features = torch.cat(tuple(self._buffer[i]), dim=1)
            selected_indices = []
    ...
    def updatebuffer(self, representation, slots, label):
        # self.sort_buffer()
        # 如果一类没在exp 中出现 buffer中会分配更多的份额 但是没新的representation 会空出份额 找其他填充（找的策略）
        re_groups = defaultdict(list)
        for i in range(len(label)):
            re_groups[label[i].item()].append(representation[i])
        for i in slots:
            if i not in self._buffer:
                self._buffer[i] = []
                for j in range(min(slots[i], len(re_groups[i]))):
                    self._buffer[i].append(re_groups[i][j])
            else:
                k = len(self._buffer[i]) - slots[i]
                while k > 0 and isinstance(self._buffer[i], list):  # 确保第i类buffer中有一个特征
                    self._buffer[i].pop(0)
                    k -= 1
                k = slots[i] - len(self._buffer[i])
                if k > 0:
                    if i not in label:
                        li = list(zip(slots.values(), slots.keys()))
                        p_li = len(li)
                        li.sort()
                        idx = bisect.bisect_right(li, (slots[i], i))
                        while idx < p_li and li[idx][1] not in label:
                            idx += 1
                        if idx == p_li:
                            idx -= 1
                            while idx >= 0 and li[idx][1] not in label:
                                idx -= 1
                        n = len(re_groups[li[idx][1]])
                        index = list(i for i in range(n))
                        np.random.shuffle(index)
                        if li[idx][1] not in self._buffer:
                            self._buffer[li[idx][1]] = []
                        for id_k in range(k):
                            if index[id_k] >= len(re_groups[li[idx][1]]):
                                continue
                            self._buffer[li[idx][1]].append(re_groups[li[idx][1]][index[id_k]])
                    else:
                        n = len(re_groups[i])
                        index = list(i for i in range(n))
                        np.random.shuffle(index)
                        for id_k in range(k):
                            self._buffer[i].append(re_groups[i][index[id_k]])
                for num in range(min(len(self._buffer[i]), len(re_groups[i]))):
                    self._buffer[i][num] = re_groups[i][num]
