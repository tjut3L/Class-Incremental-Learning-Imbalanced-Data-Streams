    ...
    **Evaluation loop**
    ...
    def eval_epoch(self, **kwargs):
        ...
        for self.mbatch in self.dataloader:
            ...
            """ 新加 """
            self.eval_output_list= self.forward()
            # self.mb_output = self.eval_output_list["logits"]
            self.mb_output = self.eval_output_list[0]
    ...
    def _before_training_exp(self, **kwargs):
        ...
        self.train_dataset_adaptation(**kwargs)  # dataset 加入新的转换
    ...
    def _train_exp(
        ...
        for _ in range(self.train_epochs):
            self._before_training_epoch(**kwargs)  # self.index = list(range(self.mem_size))

            if self._stop_training:  # Early stopping
                self._stop_training = False
                break

            # representation, exp_y = self.training_epoch(**kwargs)
            self.training_epoch(**kwargs)
            self._after_training_epoch(**kwargs)  # pass
