    ...
    **Evaluation loop**
    ...
    def train(
        ...
        **kwargs,
        ...
        if not isinstance(experiences, Iterable):   #expriences 不是迭代器即为 单个exprience
            experiences = [experiences]
        if eval_streams is None:
            eval_streams = [experiences]

        self._eval_streams = _group_experiences_by_stream(eval_streams)    #把expriences 按照name分组

        self._before_training(**kwargs)

        for self.experience in experiences:
            self._before_training_exp(**kwargs)  # 修改训练的epoch数
            self._train_exp(self.experience, eval_streams, **kwargs)
            # self.plugins[4].updatebuffer1(self, representation, exp_y)
